...
public class BaseTest2 {
	...
	
	{
		...
		{
			// invoking headless method
			ChromeOptions options = new ChromeOptions();
			WebDriverManager.chromedriver().setup();
			// if we are running headless then go inside and run
			if(browsername.contains("Headless"))
			{
			options.addArguments("Headless");
			}
			// don't want to run headless
			driver = new ChromeDriver(options);
			// we are running browser in full mode(maximize)
			// maximize option won't effect headless mode
			// so we have to give dimension
